...
void BiasRemoverFilter::cbMag(const message_filters::MessageEvent<Field const>& event) {
  const auto maybe_mag_unbiased = remover_->removeBias(*event.getConstMessage());
  if (!maybe_mag_unbiased.has_value()) {
    const auto& log = node_.get_node_logging_interface();
    const auto& clock = node_.get_node_clock_interface();
    RCLCPP_ERROR_SKIPFIRST_THROTTLE(log->get_logger(), *clock->get_clock(), 10000.,
      "Bias remover cannot work: %s. Waiting...", maybe_mag_unbiased.error().c_str());
    return;
  }

  const auto stamp = event.getReceiptTime();
  signalMessage(message_filters::MessageEvent<Field const>(
    std::make_shared<Field const>(*maybe_mag_unbiased), stamp, false, message_filters::DefaultMessageCreator<Field>()));
}

void BiasRemoverFilter::cbBias(const message_filters::MessageEvent<Field const>& event) {
  remover_->setBias(*event.getConstMessage());
}
