...
TEST_F(VisualizeAzimuthNodelet, Basic) {  // NOLINT
  ...
  auto az_pub = node->create_publisher<Az>("visualize_azimuth/azimuth", rclcpp::SystemDefaultsQoS(pub_qos));
  pubs.push_back(az_pub);
  ...
  auto vis_sub = node->create_subscription<Pose>("visualize_azimuth/azimuth_vis", qos, pose_cb);
  subs.push_back(vis_sub);
  ...
  az_pub->publish(azimuth);

  for (size_t i = 0; i < 50 && !last_pose.has_value() && rclcpp::ok(); ++i) {
  ...
  EXPECT_EQ(0, last_pose->pose.covariance[0]);
  EXPECT_EQ(0, last_pose->pose.covariance[35]);
  EXPECT_EQ(0, last_pose->pose.pose.position.x);
  EXPECT_EQ(0, last_pose->pose.pose.position.y);
  EXPECT_EQ(0, last_pose->pose.pose.position.z);
  EXPECT_EQ(0, last_pose->pose.pose.orientation.x);
  EXPECT_EQ(0, last_pose->pose.pose.orientation.y);
  EXPECT_NEAR(M_SQRT1_2, last_pose->pose.pose.orientation.z, 1e-6);
  EXPECT_NEAR(M_SQRT1_2, last_pose->pose.pose.orientation.w, 1e-6);

  last_pose.reset();
  azimuth.azimuth = M_PI_2;
  az_pub->publish(azimuth);

  for (size_t i = 0; i < 10 && !last_pose.has_value() && rclcpp::ok(); ++i) {
    executor.spin_all(100ms);
  }
  ASSERT_TRUE(last_pose.has_value());
  EXPECT_EQ(time, last_pose->header.stamp);
  ...
  EXPECT_EQ(0, last_pose->pose.pose.orientation.x);
  EXPECT_EQ(0, last_pose->pose.pose.orientation.y);
  EXPECT_NEAR(0, last_pose->pose.pose.orientation.z, 1e-6);
  EXPECT_NEAR(1, last_pose->pose.pose.orientation.w, 1e-6);
  ...
  for (size_t i = 0; i < 5 && !last_pose.has_value() && rclcpp::ok(); ++i) {
    executor.spin_all(100ms);
  }
  ...
  EXPECT_EQ(0, last_pose->pose.pose.orientation.x);
  EXPECT_EQ(0, last_pose->pose.pose.orientation.y);
  EXPECT_NEAR(-0.671109, last_pose->pose.pose.orientation.z, 1e-6);
  EXPECT_NEAR(0.741358, last_pose->pose.pose.orientation.w, 1e-6);
}
